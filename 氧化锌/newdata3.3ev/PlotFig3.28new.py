...
def PlotNex(wavelength):
    ...
    meannex = []
    for a in names.get('A' + str(wavelength)):
        file = np.loadtxt("./nex_%snm/A%s/n_ex" % (wavelength, a))
        t = file[:, 1]
        nex = file[:, 2]
...
names = locals()
names['A' + str(800)] = [20,25,30,35,35,40,50,70,100]
names['A' + str(3500)] = [10,20,25,30,35,40,50,70,100]
names['FWHM' + str(800)] = 0.585 * 500# atom unit [hbar/Ha]   1/e max
names['FWHM' + str(3500)] = 0.585*1200
names['start' + str(800)] = int(500/2.5)
names['start' + str(1300)] = int(1200/2.5)
##################   Main Program    ######################################
WaveLen = 800
