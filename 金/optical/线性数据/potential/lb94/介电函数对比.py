...
# 画出tddft结果
start, end = 1, 6
Omega = np.arange(start, end, 0.1)  # eV
...
Aprobe= cx.ReadOne("./gauge_field" , 2)
Eprobe = -1*cx.ReadOne("./gauge_field", 5) / cx.c_au # E= -1/c  dA/dt  原子单位c=137
...
epsilon = op.epsilon_kick(prop_time = time, kick_delay = 0, kick_amplitude=A0,
        induced_efield = Eprobe, damping_factor = 0.3, frequency_range = Omega_au)

plt.subplot(2,1, 1)
plt.plot(Wavelength, epsilon.imag, "-", label="TDDFT" )
plt.ylabel("$Im(\epsilon)$", fontsize=fs)

plt.subplot(2,1, 2)
plt.plot(Wavelength, epsilon.real, "-", label="TDDFT" )
