...
# path = ["pseudodojo_lda","pseudodojo_pbe","pseudodojo_pbesol", "lb94"]   # 模拟路径
path = ["lb94"]
damping= 0.0
fs = 12
plt.figure()
# 画出tddft结果
start, end = 1.2, 3.1 #   0.6, 6
...
for i in path:
        ...
        fdamp = op.damping(prop_time = time,  kick_delay = 0, damping_factor = damping)
        epsilon = op.epsilon_kick(prop_time = time, kick_delay = 0, kick_amplitude=A0,
                induced_efield = Eprobe, damping_factor = damping, frequency_range = Omega_au)
